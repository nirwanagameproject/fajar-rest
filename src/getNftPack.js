...
async function getNftPack(setPacksL,setStatusContent,setJudul,timerCooking,setAlert){
  ...
  fetch(
    "https://aa-testnet.neftyblocks.com/atomicmarket/v1/assets?page=1&limit=10&order=desc&sort=transferred&owner="+new String(global.config.session.auth.actor)+"&collection_name=fajarmuhf123&schema_name=packs"
  ).then((res) => res.json())
  .then((json) => {
